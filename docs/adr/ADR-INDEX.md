...
| [0016](0016-metrics-endpoint.md) | Prometheus `/metrics` Endpoint | Accepted | In-tree Prometheus text-format emitter on `/metrics`. v0.3 publishes daemon-level gauges (uptime, MemStats, goroutines) + scrape counter; per-tool counters and duration histograms deferred to v0.4. No new dependency. |
