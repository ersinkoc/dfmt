...
| [0021](0021-single-binary-self-promotion.md) | Single Binary, Self-Promoting Daemon | Accepted | Every `dfmt` subcommand self-promotes via `daemon.PromoteInProcess` when no daemon is running. `client.NewClient`'s `exec.Command` auto-spawn retired in v0.6.1. One `dfmt.exe` in `tasklist`. Short-lived-command terminal detach (`FreeConsole` / `setsid+fork`) deferred to v0.6.x. |
