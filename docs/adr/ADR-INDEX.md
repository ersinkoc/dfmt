...
| [0021](0021-single-binary-self-promotion.md) | Single Binary, Self-Promoting Daemon | Accepted | Every `dfmt` subcommand self-promotes via `daemon.PromoteInProcess` when no daemon is running. `client.NewClient`'s `exec.Command` auto-spawn retired in v0.6.1. One `dfmt.exe` in `tasklist`. Terminal-detach helper explicitly **decided against** in v0.6.2 — the OS-level fork-and-exit it would require partially reverses the no-spawn commitment, and the typical workflow (Claude Code → `dfmt mcp` self-promotes → other invocations connect-and-exit) already delivers non-blocking UX without it. |
