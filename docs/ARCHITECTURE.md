...
| 0016 | Prometheus `/metrics` Endpoint         | In-tree Prometheus text-format emitter on `/metrics`. v0.3 publishes daemon-level gauges (uptime, MemStats, goroutines) + scrape counter; per-tool counters deferred to v0.4. No new dependency. |
